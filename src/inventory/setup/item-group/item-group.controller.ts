import { Controller, Get, UseGuards, Post, Patch, HttpStatus, Param, Body } from '@nestjs/common';
...
export class ItemGroupController {
    ...
    @UseGuards(AuthGuard('jwt'))
    @Post()
    async create(@Body() dto: CreateItemGroup, @AuthUserInfo() authUserInfo: Users) {
        try {
    ...
    @UseGuards(AuthGuard('jwt'))
    @Patch()
    async update(@Param('id') id: number, @Body() dto: UpdateItemGroup, @AuthUserInfo() authUserInfo: Users) {
        try {
            const response = await this.itemGroupService.update(id, dto, authUserInfo)

...
export class ModulelinksassignController {
    constructor(private readonly moduleLinkAssingService: ModulelinksassignService) {}
    ...
    @Post()
    async create(@Body() body: CreateModuleLinkAssing, @AuthUserInfo() authUserInfo: Users) {
        try {
            const response = await this.moduleLinkAssingService.createModuleAssign(body, authUserInfo)
    ...
    @UseGuards(AuthGuard('jwt'))
    @Patch(':id')
    async update(@Param('id') id: number, @Body() body: UpdateModuleLinkAssing, @AuthUserInfo() authUserInfo: Users) {
        try {
            const response = await this.moduleLinkAssingService.updateModuleAssign(id, body, authUserInfo)

import { Get, Post, Body, Controller, HttpStatus, BadRequestException, UseGuards, Patch, Param } from '@nestjs/common';
...
export class ModulelinksController {
    ...
    @UseGuards(AuthGuard('jwt'))
    @Post()
    async create(@Body() body: CreateModuleLinkDto, @AuthUserInfo() authUserInfo: Users) {
        try {
            const response = await this.moduleLinkService.createModuleLink(body, authUserInfo)
            return { message: "Create Successfully", status: HttpStatus.OK, response}
        } catch (error) {
    ...
    @UseGuards(AuthGuard('jwt'))
    @Patch(':id')
    async update(@Param('id') id: number, @Body() body: UpdateModuleLinkDto, @AuthUserInfo() authUserInfo: Users) {
        try {
            const response = await this.moduleLinkService.update(id, body, authUserInfo)

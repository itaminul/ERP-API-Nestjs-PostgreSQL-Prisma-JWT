...
export class ReligionsController {
    ...
    @UseGuards(AuthGuard('jwt'))
    @Post()
    async create(@Body() dto: CreateReligionDto, @AuthUserInfo() authUserInfo: Users) {
        try {
    ...
    @UseGuards(AuthGuard('jwt'))
    @Patch(':id')
    async update(@Param('id') id: number, @Body() dto: UpdateReligionDto, @AuthUserInfo() authUserInfo: Users) {
        try {

...
export class BloodGroupsController {
    ...
    @UseGuards(AuthGuard('jwt'))
    @Post()
    async create(@Body() dto: CreateBloodGroupsDto, @AuthUserInfo() authUserInfo: Users) {
        try {
    ...
    @UseGuards(AuthGuard('jwt'))
    @Patch(':id')
    async update(@Param('id') id: number, @Body() dto: UpdateBloodGroupsDto, @AuthUserInfo() authUserInfo: Users) {
        try {
            const results = await this.bloodGroupService.update(id, dto, authUserInfo)

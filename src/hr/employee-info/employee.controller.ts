...
export class EmployeeController {
  ...
  async getEmployeeByDateWise(@Body() body) {
    try {
      ...
      const response = await this.employeeService.getEmployeeByDateWise(fDate, tDate)
      return { message: "Show Successfully", status: HttpStatus.OK, response }
    } catch (error) {
      if (error instanceof Prisma.PrismaClientKnownRequestError) {
  ...
  async updateEmpInfo(@Param('id') id: number, @Body() body: CreateEmployeeDto, @AuthUserInfo() authUserInfo: Users, @UploadedFile() file: Express.Multer.File) {
    try {
      ...
      const resizedImagePath = resizedFilePath
      const empPhoto = resizedImagePath
      const response = await this.employeeService.updateEmpInfo(id, body, authUserInfo, empPhoto)
      return { message: "Insert Successfully", status: HttpStatus.OK, response }
    } catch (error) {
      if (error instanceof Prisma.PrismaClientKnownRequestError) {
